...
export class ForestScene extends Phaser.Scene {
  ...
  preload(){
    this.load.image('forest',forestURL);this.load.image('atlas','./v3/tata-atlas.png');this.load.image('blink-atlas','./v3/tata-blink.png');this.load.image('logo','./tattu-logo.png');this.load.image('hero','./yellow-wolf-runner.png');
  ...
  decorate(node,cell){
    ...
    const glow=this.add.graphics();glow.lineStyle(3,cell.special==='nova'?0xffdf65:0xf5ffff,.9).strokeCircle(0,0,32);glow.lineStyle(2,COLORS[cell.kind],.7).strokeCircle(0,0,36);node.badge.add(glow);
    const plate=this.add.circle(23,23,14,cell.special==='nova'?0xffba24:0x3691a8).setStrokeStyle(2,0xfffce5);node.badge.add(plate);
    node.badge.add(this.text(23,22,{row:'↔',column:'↕',bomb:'✹',nova:'★'}[cell.special],23,'#fffce5'));
  }
  drawControls(){
    this.toolObjects={};
    this.toolSelection=this.add.graphics().setDepth(28).setVisible(false);this.toolSelection.lineStyle(4,0xffda39).strokeRoundedRect(-62,-46,124,93,24);this.toolSelection.lineStyle(2,0xffffff).strokeRoundedRect(-65,-49,130,99,27);
    const specs=[['hammer','电能锤',134,'hammer'],['bomb','无人机',292,'drone'],['shuffle','重排',450,'shuffle'],['rush','超载',608,'bolt']];
    for(const [id,label,x,shape]of specs){this.panel(x-58,1025,116,85,id==='rush'?0xffe7a3:0xfff6dc,0xfff6d6,23,18);this.drawGlyph(x,1055,shape);this.text(x,1089,label,22,'#7e7548').setDepth(24);const badge=this.add.circle(x+41,1029,17,0x79ba48).setStrokeStyle(3,0xfff7d5).setDepth(25);const count=this.text(x+41,1029,id==='rush'?'⚡':this.tools[id],20,'#fff').setDepth(26);this.toolObjects[id]={badge,count};this.addHit(id==='rush'?'开启超载':label,x-58,1025,116,90,()=>this.chooseTool(id));}
    this.hintText=this.text(360,1141,'四连闪电  ·  T / L 爆炸  ·  五连彩虹',21,'#fffce1',{stroke:'#638c64',strokeThickness:3}).setDepth(20);
    this.drawGlyph(71,1178,'pause');this.drawGlyph(643,1178,'help');this.addHit('暂停与设置',38,1148,70,65,()=>this.pauseGame());this.addHit('玩法说明',608,1148,70,65,()=>this.openGuide());
    this.text(360,1187,'TATTU  黄狼极速无限消',20,'#fff9ca',{letterSpacing:1,stroke:'#537950',strokeThickness:2}).setDepth(20);
    this.addHit('查看能量',216,261,440,30,()=>{if(this.energy>=100)this.startRush();else this.say(`还差 ${100-Math.floor(this.energy)} 点能量，连消可加速充能`);});
  }
  ...
  syncAccessibility(){
    if(!this.cellButtons){
      ...
      for(let i=0;i<72;i++){
        ...
        b.onpointerup=e=>{
          ...
          if(this.activeTool||this.state==='rush'||(this.state==='resolving'&&this.rushRemaining>0)){this.select(gesture.index);return;}
    ...
    const app=document.getElementById('app');Object.assign(app.dataset,{state:this.state,score:String(this.score),moves:String(this.moves),energy:String(Math.floor(this.energy)),ice:String(this.ice.size),goals:this.progress.join(','),rush:String(Math.max(0,Math.ceil(this.rushRemaining/1000)))});
  }
  updateHUD(){
    ...
    const g=this.energyTrack;g.clear();g.fillStyle(0x638e75,.55).fillRoundedRect(206,264,390,20,10);g.fillStyle(0xeaffd5,.8).fillRoundedRect(209,267,384,14,7);g.fillStyle(this.rushRemaining?0xffaa39:0xf2ce4b).fillRoundedRect(209,267,Math.max(1,384*(this.rushRemaining?this.rushRemaining/15000:this.energy/100)),14,7);this.energyText.setText(this.rushRemaining?`${TRACKS[this.track].name}  ·  疯狂点击`:(this.energy>=100?'能量满了！点击右下角「超载」':'消除充能 · 满格开启 15 秒超载'));this.rushTimeText.setText(this.rushRemaining?`${Math.ceil(this.rushRemaining/1000)}s`:'');
    if(this.toolObjects)for(const id of['hammer','bomb','shuffle'])this.toolObjects[id].count.setText(this.tools[id]);if(this.toolObjects)this.toolObjects.rush.badge.setFillStyle(this.energy>=100||this.rushRemaining?0xf0a22e:0x8aaf88);this.syncAccessibility();
  }
  ...
  showIntro(){this.showDialog(`<img class="hero" src="./yellow-wolf-runner.png" alt="黄色闪电狼塔塔"><div class="eyebrow">TATA · ENERGY FOREST</div><h1 class="ribbon">第 1 关 · 点亮森林</h1><p>30 步内完成目标<br>和塔塔一起释放满满电力！</p>${this.goalPreview()}<button class="cta" id="start">开始闯关</button><p class="muted">滑动交换 · 三个相同即可消除</p>`);this.root.querySelector('#start').onclick=()=>this.begin();}
  ...
  async playTurn(turn,consumeMove=false,fast=false){
    if(!this.alive||!turn.frames.length)return;this.state='resolving';this.selection.setVisible(false);this.syncAccessibility();if(turn.valid&&consumeMove)this.moves--;this.updateHUD();
    for(const frame of turn.frames){if(!this.alive)return;
      if(frame.type==='swap'||frame.type==='rollback'){this.audio.swap(frame.type==='swap');await this.moveFrame(frame.board,frame.type==='swap'?160:190);if(frame.type==='rollback')this.say('再找找，三个相同的塔塔要连在一起');}
      else if(frame.type==='clear'){await this.clearFrame(frame,fast);}
      else if(frame.type==='fall'){await this.moveFrame(frame.board,fast?85:280,true);}
      else if(frame.type==='shuffle'){this.fx.floatText('能量重组！',360,610,'#fffadc',43);await this.moveFrame(frame.board,380,true);}
    }
    if(!this.alive)return;this.board=turn.board;this.state=this.rushRemaining>0?'rush':'idle';this.lastAction=this.time.now;this.updateHUD();if(!fast&&turn.valid)this.say(turn.combo>1?`漂亮！${turn.combo} 次连锁消除`:'继续收集能量，塔塔等你下一击');if(this.rushRemaining>0&&this.rushQueue.length){const next=this.rushQueue.shift();this.queueRush(next);}else if(!this.rushRemaining)this.finishCheck();
  }
  async moveFrame(board,duration,fall=false){
    const ids=new Set(board.filter(Boolean).map(c=>c.id));for(const[id,node]of this.nodes)if(!ids.has(id)){node.destroy();this.nodes.delete(id);}const motions=[];
    ...
    await Promise.all(motions);this.board=board;
  }
  async clearFrame(frame,fast){
    ...
    const aliveScore=this.score;this.score+=frame.score??0;this.energy=Math.min(100,this.energy+(frame.cleared?.length??0)*2.3+(frame.chain>1?4:0));
    if(frame.chain>1)this.fx.floatText(['','', '漂亮连击！','能量连锁！','超棒！','不可思议！'][Math.min(frame.chain,5)],360,310, '#fffad3',44);
    if(frame.activated?.length&&!fast&&!this.reduced)this.cameras.main.shake(100,.002);
    ...
    await this.delay(fast?20:100);await Promise.all(dying.filter(n=>n.active).map(n=>this.tween(n,{scale:.2,alpha:0,duration:fast?45:110}).then(()=>{this.nodes.delete(n.cell.id);n.destroy();})));
  ...
  chooseTool(tool){if(this.state!=='idle'&&this.state!=='rush')return;if(tool==='rush'){this.startRush();return;}if(this.state==='rush'){this.say('超载进行中，尽情点击棋盘！');return;}if(!this.tools[tool]){this.say('这个道具本局已用完');return;}this.clearHint();this.selected=null;this.selection.setVisible(false);if(tool==='shuffle'){this.tools.shuffle--;this.activeTool=null;this.playTurn(buildToolTurn(this.board,0,'shuffle'),false);return;}this.activeTool=this.activeTool===tool?null:tool;this.say(this.activeTool?(tool==='hammer'?'点击目标，电能锤可直接击碎冰层':'点击目标，无人机将引爆周围 3 × 3 区域'):'道具已取消');this.audio.tap();}
  startRush(){if(this.state==='rush'){this.say('超载进行中，尽情点击棋盘！');return;}if(this.state!=='idle')return;if(this.energy<100){this.say(`还差 ${100-Math.floor(this.energy)} 点能量，连消可加速充能`);this.audio.tap();const badge=this.toolObjects.rush.badge;this.tweens.killTweensOf(badge);badge.setScale(1);this.tweens.add({targets:badge,scale:1.18,duration:this.reduced?35:90,yoyo:true});return;}this.clearHint();this.activeTool=null;this.selected=null;this.selection.setVisible(false);this.energy=0;this.rushRemaining=15000;this.state='rush';this.track=(this.track+1)%10;safeWrite('tata-last-track',this.track);this.audio.startRush(this.track,({strength})=>{if(this.alive&&this.rushRemaining>0)this.fx.rushBeat(strength);});this.fx.floatText('塔塔超载！',360,590,'#ffe665',68);this.fx.floatText('15 秒 · 点击即爆',360,654,'#ffffff',32);this.say('超载开始！点击任意图标释放电力');this.updateHUD();}
  queueRush(index){if(this.rushRemaining<=0)return;if(this.state==='resolving'){if(this.rushQueue.length<6)this.rushQueue.push(index);return;}if(this.state!=='rush')return;this.playTurn(buildRushTurn(this.board,index),false,true);}
  finishCheck(){if(this.state!=='idle')return;this.rushQueue=[];if(this.progress[0]>=18&&this.progress[1]>=18&&this.ice.size===0){this.state='won';this.showResult(true);}else if(this.moves===0){this.state='lost';this.showResult(false);}else if(this.ice.size===1&&this.progress.every(v=>v>=18)&&this.tools.hammer){this.say('只剩最后一块冰啦，试试电能锤！');}this.syncAccessibility();}
  async showResult(win){this.audio.stopRush();this.rushRemaining=0;this.clearHint();this.activeTool=null;this.syncAccessibility();if(win){this.score+=this.moves*60;this.best=Math.max(this.best,this.score);safeWrite('tata-forest-best',this.best);this.audio.win();this.fx.celebrate();this.fx.floatText('森林点亮啦！',360,560,'#fff7bd',60);}else this.audio.lose();this.updateHUD();await this.delay(win?850:250);if(!this.alive)return;const stars=win?Math.max(1,[1500,4000,8500].filter(n=>this.score>=n).length):0;
  ...
  openGuide(result=false){if(!this.alive)return;if(result){if(!['won','lost'].includes(this.state)||!this.root.querySelector('#replay'))return;}else{if(['intro','starting','won','lost','paused'].includes(this.state))return;this.pauseGame();}const savedResult=result?this.root.firstElementChild:null;this.showDialog(`<h2 class="ribbon">塔塔的消除秘诀</h2><div class="guide-lines"><span>三连：</span>交换相邻塔塔，让三个同色相连。<br><span>四连：</span>合成闪电，击穿整行或整列。<br><span>T / L 连：</span>合成爆炸，清除周围区域。<br><span>五连：</span>合成彩虹，交换后清除一种颜色。<br><span>特效互换：</span>两种特效组合释放更强能量！<br><span>冰层：</span>消除上面的塔塔即可击碎。<br><span>超载：</span>满能量启动，15 秒点击即爆。</div><button class="cta" id="guide-back">知道了</button>`);this.root.querySelector('#guide-back').onclick=()=>{if(result){this.root.replaceChildren(savedResult);this.root.querySelector('#result-guide')?.focus();}else this.resumeGame();};}
  restart(){this.previewAudio?.destroy();this.audio.destroy();this.root.replaceChildren();this.scene.resume();this.scene.restart();}
  update(time,delta){if(!this.alive)return;
    this.toolSelection?.setVisible(!!this.activeTool);if(this.activeTool)this.toolSelection?.setPosition(this.activeTool==='hammer'?134:292,1067);
    if(this.rushRemaining>0&&this.state!=='paused'){this.rushRemaining=Math.max(0,this.rushRemaining-Math.min(delta,100));if(!this.rushRemaining){this.audio.stopRush();this.rushQueue=[];if(this.state==='rush'){this.state='idle';this.finishCheck();}this.say('超载结束，继续完成森林任务');}if(!this.board.some(c=>!c))this.updateHUD();}
